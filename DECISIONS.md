...
### M6 Propose (gated NL write) choices (recorded as built) — the human-in-the-loop gate
- **The only write path is a `SchedulerRequest`** (`expedite` / `block_resource`) — a structured, serializable object. The agent never mutates the schedule; it emits a request, and applying it is gated. This is the project's central thesis as code: AI proposes, the constraint solver enforces, a human disposes.
- **`preview()` re-solves inside a transaction that always rolls back.** The impact (slipped lots, tardiness delta, feasibility) is computed by actually applying the request + re-solving with CP-SAT, then a sentinel exception rolls the savepoint back — so a preview *provably* persists nothing (the mutation and the re-solved schedule both vanish; only the plain-data diff survives). Verified: after preview, the job is untouched and the board is the same schedule row.
- **`approve()` refuses to commit an infeasible result.** It applies + re-solves in an atomic block; if the re-solve is infeasible it raises and the block rolls back — you can never commit a plan the solver can't satisfy. "The AI can never produce an infeasible schedule" thus extends to "you can't approve one either."
- **The agent's propose tool previews; it does not apply.** `propose_schedule_change` runs `preview` (rollback) and hands the impact back to the agent so it can recommend `approve`/`caution`/`reject` honestly — surfacing the "looks free but slips two others" trade (challenge #6). The agent's toolset is read tools + this one gated propose tool + a terminal `present_proposal`; there is no direct-mutation tool.
- **Reuses M4's `resolve.py`** (`diff_schedules`, `weighted_tardiness`) — `SchedulerRequest.apply()` performs the same mutations as the M4 disruptions, but behind the preview/approve gate.
- **Full audit trail:** `preview` logs `PROPOSAL`, `approve` logs `APPROVAL` (with the new schedule id), `reject` logs `REJECTION`, and a refused-infeasible approval logs a `PROPOSAL` marked REFUSED. Infeasibility is proven in tests by injecting a stub solver (the request kinds themselves can't create infeasibility, by the same finite-availability argument as M3).

### M7 UI choices (recorded as built)
- **Aesthetic: an industrial control-room / blueprint look**, not the generic light/serif default. Deep-ink background with a faint blueprint grid, **IBM Plex Sans + Mono** (engineering-flavored, deliberately not Inter/Roboto), a **safety-amber** accent for AOG and primary actions, **mint** for feasible/on-time, **red** for late/reject, and mono for all data. The tone matches a serious shop-floor tool for an ops-literate audience. Built with the `frontend-design` skill.
- **Django templates + HTMX + Plotly, server-rendered.** Plotly figures (Gantt via `px.timeline`, utilization bars) are built server-side in `plant/charts.py` from a persisted `Schedule` and embedded as divs; HTMX swaps the Ask/Propose result partials without a full reload. No JS build step — fits the Django stack and keeps the demo deployable as one image.
- **The views are thin.** They render data and wire the browser to the already-tested agent (`plant.agent`) and gate (`plant.propose`) layers; the Approve/Reject buttons carry the serialized `SchedulerRequest` (JSON in a hidden field) from the preview POST to the approve POST. No scheduling or safety logic lives in the view layer.
- **Graceful AI degradation.** Ask/Propose check `settings.ANTHROPIC_API_KEY` and show a "set the key" notice when absent, so the board, solver, baseline-vs-CP-SAT toggle, and audit trail are fully usable with no key. The key is passed through `docker-compose` from the host shell for local live testing; it's a Render secret in prod.
- **Per PLAN, the UI is demonstrated by the recording, not chased with coverage** — just four view smoke tests (pages render; Ask/Propose degrade without a key). The board is now the landing page (the M0 self-check page is retired; `/healthz` still runs those checks for Render).
- **Verified live in a browser** (Playwright, against Postgres): the Gantt and utilization render, the CP-SAT/baseline and resource/job toggles work, and the HTMX+CSRF Ask flow round-trips end to end.

### M8 polish / deploy choices (recorded as built)
- **Deploy = a Render Blueprint (`render.yaml`): one Docker image + a managed Postgres.** The image runs `collectstatic` at build, migrates on boot, and serves via gunicorn with a `/healthz` health check. `DJANGO_SECRET_KEY` is Render-generated; `DATABASE_URL` is wired from the managed DB.
- **The API key is never in the repo.** `ANTHROPIC_API_KEY` is `sync: false` in `render.yaml` — entered once in the Render dashboard (or absent). Scanned the working tree *and* full git history: no `sk-ant-` value anywhere; `.env`/`.env.*` are gitignored. The key lives only in the host shell (local) or the dashboard (prod).
- **The deploy is fully demoable without a key.** Because Ask/Propose degrade gracefully, the board, both solvers, the toggles, and the audit trail work the moment the service is live; the AI layer turns on the instant the key is added — no redeploy of the image, just an env var.
- **Prod runtime verified locally** by running the built image with gunicorn + `DEBUG=0` + whitenoise static against Postgres (not the compose `runserver` override): `/healthz` green, board 200, migrations applied on boot — i.e. exactly what Render runs.
- **Cost guard:** the agent is on-demand (button-triggered), prompt-cached (frozen system prompt), and `max_tokens`-capped — AI spend is bounded and only incurred when a planner asks or proposes.
- **`README.md` + `DEPLOY.md`** carry the what/why, one-command local run, architecture, a board screenshot, the security/synthetic-data disclaimers, and the step-by-step Render runbook. The live integration run of Ask/Propose against the real API happens when the key is added in the dashboard (deferred at the owner's request until key handling is confirmed safe).
