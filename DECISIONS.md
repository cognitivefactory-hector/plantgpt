...
- **Horizon:** total serial work + latest maintenance end + 1, a safe upper bound on any schedule; tardiness vars get headroom for already-past-due jobs.
- **Scope note — shifts:** worker *shift time-of-day windows* are not yet enforced; workers are treated as available across the horizon (certifications + single-tasking ARE enforced). This is the next M3 step, planned via per-worker off-shift "blocker" intervals reusing the same no-overlap machinery.
